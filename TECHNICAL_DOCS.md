...
*   **Гибридный ввод:**
    ...
    *   **Текст:** Полноценный UI для ввода текста с валидацией (проверка на наличие кириллицы).
...
Приложение является одностраничным (SPA). Точка входа — `index.tsx`.
*   **Адаптивность:** Используются переменные CSS Telegram (`var(--tg-content-safe-area-inset-top)`) для корректного отображения на всех устройствах.
*   **Состояния приложения (`AppState`):**
    *   `IDLE`: Ожидание ввода.
    *   `LISTENING`: Активен микрофон, идет запись.
    *   `PROCESSING`: Запрос отправлен в n8n, ожидание ответа.
