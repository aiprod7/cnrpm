...
> 🎉 **Новое в v1.0.4**: Исправлена проблема повторных запросов разрешения микрофона! Теперь в рамках одной сессии разрешение запрашивается только один раз.  
> ⚡ **Особенность**: Нативный аудио-режим с минимальной задержкой транскрибации!  
...
- **🎯 Умное кеширование микрофона** (NEW v1.0.4): 
  - ✅ **Один запрос разрешения за сессию** вместо множественных
  - ✅ Кэширование MediaStream для переиспользования
  - ✅ Проверка через Permissions API перед запросом
  - ✅ Интеграция с Telegram Web App SDK (viewport, popup события)
  - 📖 Подробнее в [MICROPHONE_PERMISSION_FIX.md](MICROPHONE_PERMISSION_FIX.md)
